...
let pp_ident fmt x =
  pp_print_text fmt String.(concat "__" @@ split_on_char '#' x)
...
| ArrayMapBy(n,x,ty,e) ->
   ...
   fprintf fmt "(let %a : %a array = %a in@," pp_ident y print_ty ty pp_exp e;
...
| ArrayFoldLeft(x,ty1,ty2,acc,e) ->
    fprintf fmt "(Array.fold_left %a (%a : %a) (%a : %a array))" 
      pp_ident x 
      pp_exp acc 
      print_ty ty1 
      pp_exp e
...
| ListFoldLeft(x,ty1,ty2,acc,e) ->
    fprintf fmt "(List.fold_left %a (%a : %a) (%a : %a list))" 
      pp_ident x 
      pp_exp acc 
      print_ty ty1 
      pp_exp e
