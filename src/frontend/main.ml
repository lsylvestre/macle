...
let () =
 ...
 in
 Arg.parse [
      ("-time",            Arg.Set Gen_platform.print_compute_time, 
                           "instrument the produced code to measure the execution time");
      ("-time-short",      Arg.Set Gen_platform.print_compute_time_short,
                           "variant of the -time option");
      ("-show-ast",         Arg.Set flag_show_ast, 
                            "display the syntax tree (AST) of the input program");
      ("-show-typed-ast",   Arg.Set flag_show_typed_ast, 
                            "display the *typed* AST of the input program");
      ("-show-inlined-ast", Arg.Set flag_show_inlined_ast, 
                            "display the typed AST of the input program *after inlining*");
      ("-show-kast",        Arg.Set flag_show_kast, 
                            "display the *kernel AST* (VSML), an automaton-based representation");
      (* ************** *)
...
let parse filename = 
  ...
  try 
    ...
    | PLATFORM -> 
        ...
        if !Gen_platform.print_compute_time then
