...
(* [(stack_size_env*32)] must be a multiple of the size 
   of the RAM blocs available on the targeted Board *)
let stack_size_env = ref 3072;; 
...
let rec c_instruction env (dst:ident) fmt (s:inst) : unit =
  ...
  | ESML_stackPrim c ->
      ...
      (match c with
       | Push((a,t),q) ->
          fprintf fmt "stack_ptr <= stack_ptr + 1;@,";
          fprintf fmt "stack(stack_ptr + 1) <= unsigned(%a);@," (c_atom env) (Atom.Prim(ToCaml t,[a]));
           assign env fmt dst c_state q
      | LetPop ((x,t),q) -> 
          fprintf fmt "%a <= " c_ident x;
          from_caml (fun fmt () -> fprintf fmt "stack_top") fmt () t;
          fprintf fmt ";@,";
          fprintf fmt "stack_ptr <= stack_ptr - 1;@,";
          assign env fmt dst c_state q
...
let compile_esml_circuit ?(reset="reset") ?(clock="clk")
  ...
  List.iter2 (fun sv (ts,e)(* automaton *) ->
    ...
    if !allow_stack then (
      fprintf fmt "@,-------------------------------------------------------------------@,"; 
      fprintf fmt "-- stack designed to be implemented with RAM blocks @,";      
      fprintf fmt "type stack_T is array (0 to %d) of unsigned(31 downto 0);@," (!stack_size_env-1);
      fprintf fmt "signal stack : stack_T;@,";
      fprintf fmt "signal stack_top : caml_value;@,";
      fprintf fmt "signal stack_ptr : integer range 0 to %d := 0;@," (!stack_size_env - 1);
      fprintf fmt "-------------------------------------------------------------------@,@,";   
  ...
  if !allow_stack then begin
    fprintf fmt "-------------------------------------------------------------------@,";
    fprintf fmt "-- only one read of the top of the the stack to allow RAM inference@,";
    fprintf fmt "stack_top <= std_logic_vector(stack(stack_ptr));@,";
    fprintf fmt "-------------------------------------------------------------------@,";
  end;
