...
let rec c_instruction env dst fmt s = 
  match s with
  | ESML_atom a -> 
      assign env fmt dst (c_atom env) a
  | ESML_continue q -> 
...
let c_transition (dst:state) fmt (q,s) = 
  fprintf fmt "@[<v 2>when %a =>@,%a@]" 
    c_state q
    (c_instruction [] dst) s

let c_transitions (dst:state) fmt ts = 
  pp_print_list
