...
let t_val ty fmt x =
 let open Ktypes in
  ...
  | TConst TInt -> fprintf fmt "Int_val(%s)" x
  | TConst TUnit -> fprintf fmt "Bool_val(%s)" x (* I use Bool_val to construct unit value *)
...
let mk_vhdl_with_cc ?labels ?xs_opt circuit =
  let open Kast in
  let ESML.{x=name;s} = circuit in
  let envi = List.filter_map (function (In,x,t) -> Some (x,t) | _ -> None) s in
  let envo = List.filter_map (function (Out,x,t) ->  Some (x,t) | _ -> None) s in
  let envl = List.filter_map (function (Local,x,t) ->  Some (x,t) | _ -> None) s in
